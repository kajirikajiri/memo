...
const Home: NextPage = () => {
  ...
  const firebaseSignIn = async()=>{
    ...
    .then((result) => {
      ...
      snackbar.showMessage('ログインに失敗しました', 'close', ()=>{})
    });
  }
  ...
  return (
    ...
    <Box display="flex" flexDirection="column" justifyContent="center" width="100%" alignItems="center">
      <Box width="100%" display="flex" justifyContent="flex-end">
        <Button onClick={handleChangeSignType}>{!isSignIn?'ログイン':'登録'}</Button>
      </Box>
      <Box width={200}>
        <Button startIcon={<Email/>} fullWidth variant="contained" onClick={handleOpen}>emailで{isSignIn ? 'ログイン':'登録'}</Button>
        <Box height={10}></Box>
        <Button startIcon={<Google/>} fullWidth variant="contained" onClick={firebaseSignIn}>Googleで{isSignIn?'ログイン':'登録'}</Button>
        <Box height={10}></Box>
        <Button fullWidth onClick={handleOpenForgotPassword}>パスワードを忘れた</Button>
      </Box>
