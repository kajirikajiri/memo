...
export const SignUp: NextPage = () => {
  ...
  const handleSubmit = async(e:React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      snackbar.showMessage('登録に失敗しました', 'close', ()=>{})
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Box display="flex" flexDirection="column" alignItems="center" component="form" onSubmit={handleSubmit}>
          <TextField inputRef={emailRef} required type="email" label="email" defaultValue="" inputProps={{pattern: emailRegex }} />
          <Box height={10}></Box>
          <PasswordChecker password={password}/>
          <TextField inputRef={passwordRef} required type="password" label="password" defaultValue="" inputProps={{pattern: passwordRegex}} onFocus={()=> setPasswordFocus(true)} onChange={handleChange} />
          <Box height={20}></Box>
